...
/// Handler for the `ext.aitest.tap` VM Service extension.
///
...
///
/// Parameters:
/// - `ref` (required): opaque ref string from a prior `ext.aitest.snapshot` call.
///
/// Response JSON:
/// ```json
/// { "ref": "e3" }
...
///
/// Parameters:
/// - `ref` (required): opaque ref string from `ext.aitest.snapshot`.
///
/// Response JSON:
/// ```json
/// { "ref": "e5" }
...
Future<developer.ServiceExtensionResponse> aiTestHoverHandler(
  ...
  try {
    ...
    if (ref == null || ref.isEmpty) {
      return developer.ServiceExtensionResponse.error(
        developer.ServiceExtensionResponse.extensionError,
        'ext.aitest.hover: missing required param "ref"',
      );
    }
    ...
    if (entry == null) {
      return developer.ServiceExtensionResponse.error(
        developer.ServiceExtensionResponse.extensionError,
        'ext.aitest.hover: ref "$ref" not found in registry',
      );
    }
    ...
    developer.log(
      '[ai-test-v3] ext.aitest.hover: unexpected error: $e\n$st',
      name: 'ai-test',
    );
    return developer.ServiceExtensionResponse.error(
      developer.ServiceExtensionResponse.extensionError,
...
/// Handler for the `ext.aitest.drag` VM Service extension.
///
...
Future<developer.ServiceExtensionResponse> aiTestDragHandler(
  ...
  try {
    ...
    if (startRef == null || startRef.isEmpty) {
      return developer.ServiceExtensionResponse.error(
        developer.ServiceExtensionResponse.extensionError,
        'ext.aitest.drag: missing required param "startRef"',
      );
    }
    if (endRef == null || endRef.isEmpty) {
      return developer.ServiceExtensionResponse.error(
        developer.ServiceExtensionResponse.extensionError,
        'ext.aitest.drag: missing required param "endRef"',
      );
    }
    ...
    if (startEntry == null) {
      return developer.ServiceExtensionResponse.error(
        developer.ServiceExtensionResponse.extensionError,
        'ext.aitest.drag: startRef "$startRef" not found in registry',
      );
    }
    ...
    if (endEntry == null) {
      return developer.ServiceExtensionResponse.error(
        developer.ServiceExtensionResponse.extensionError,
        'ext.aitest.drag: endRef "$endRef" not found in registry',
      );
    }
    ...
    developer.log(
      '[ai-test-v3] ext.aitest.drag: unexpected error: $e\n$st',
      name: 'ai-test',
    );
    return developer.ServiceExtensionResponse.error(
      developer.ServiceExtensionResponse.extensionError,
...
/// during [AiTestPluginV3.install]. This module uses [registerExtensionIdempotent]
...
/// - `ext.aitest.tap` — pointer Down+50ms+Up at ref center; requestKeyboard for text fields.
/// - `ext.aitest.hover` — PointerHoverEvent (mouse kind) at ref center.
/// - `ext.aitest.drag` — Down+5×Move+Up from startRef to endRef.
