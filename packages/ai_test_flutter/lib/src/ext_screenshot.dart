  ...
  try {
    ...
    if (rectParam != null && rectParam.isNotEmpty && subRect == null) {
      return developer.ServiceExtensionResponse.error(
        ...
        'ext.aitest.screenshot: malformed rect "$rectParam". '
...
_CaptureTarget _resolveCaptureTarget({
  ...
  if (boundaryLayer is! OffsetLayer) {
    throw StateError(
      'ext.aitest.screenshot: repaint-boundary ancestor has no OffsetLayer '
...
RenderObject _findRepaintBoundaryAncestor(RenderObject start) {
  ...
  throw StateError(
    'ext.aitest.screenshot: no repaint-boundary ancestor reachable from '
...
/// Preference order:
///
/// 1. [AiTestPluginV3.rootRepaintBoundaryKey] when its `currentContext`
...
RenderObject _resolveRootRenderObject() {
  ...
  if (rootRO == null) {
    throw StateError(
      'ext.aitest.screenshot: root element has no render object attached. '
...
RenderObject _resolveRefRenderObject(String ref) {
  ...
  if (ro == null) {
    throw StateError(
      'ext.aitest.screenshot: ref "$ref" resolved to an entry with no live '
